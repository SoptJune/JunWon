...
fun main() {
    val (_n, _m) = baekjun.tree.`4803`.br.readLine().split(" ").map{it.toInt()}
    baekjun.n = _n; m = _m
    repeat(baekjun.n) { i ->
        graph[i] = baekjun.tree.`4803`.br.readLine().split(" ").map{it.toInt()}.toMutableList()
    }
    var ans = 0
    while(!isAllZero()) {
        ans++
...
fun dfs(x: Int, y: Int, num: Int) {
    ...
    repeat(4) { i ->
        val nx = x + dx[i]
        val ny = y + dy[i]
        if(nx in 0 until baekjun.n && ny in 0 until m && graph[nx][ny] == 0 && area[nx][ny] == -1) {
            dfs(nx,ny,num)
        }
    }
}
    ...
    repeat(baekjun.n) { x ->
        repeat(m) { y ->
            if(graph[x][y] == 1){
                ...
                val nx = x + dx[i]
                val ny = y + dy[i]
                if(nx in 0 until baekjun.n && ny in 0 until m && graph[nx][ny] == 0 && area[nx][ny] == 0) cnt++
                }
